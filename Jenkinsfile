pipeline {
    ...
    stages {
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag box-game yash5090/box-game:latest " 
                        //}
                    }
                }
            }
        ...
        stage('Docker Image Scanning') { 
            steps { 
                sh "trivy image --format table -o trivy-image-report.html yash5090/box-game:latest" 
            } 
        } 
        ...
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/box-game:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/box-game:latest'
                       sh 'docker-scout cves yash5090/box-game:latest'
                       sh 'docker-scout recommendations yash5090/box-game:latest'
                       sh 'docker-scout attestation yash5090/box-game:latest'
                   }
                }   
            }
        }
        ...
        stage("TRIVY"){
            steps{
                //dir('Band Website') {
                    sh "trivy image yash5090/box-game:latest > trivyimage.txt"   
                //}
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop box-game | true"
                sh "docker rm box-game | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh 'docker run -d --name box-app -p 5000:80 yash5090/box-game:latest' 
                //}
            }
        }
